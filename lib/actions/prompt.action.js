...
const populateUser = (query) => query.populate({
    path: 'author',
    model: User,
    select: '_id username email clerkId'
})
...
export async function deleteAllPromptsByUserId(userId) {
    try {
        await connectToDatabase(); // Connect to MongoDB
        console.log('heyyyyy', userId)
       
       // Delete all prompts where the author field matches the userObjectId
       const result = await Prompt.deleteMany({ author: userId.toHexString() });
       console.log('result',  result)

       return result.deletedCount; // Return the number of deleted prompts
    } catch (error) {
        // handleError(error);
        console.log(error)
    }
