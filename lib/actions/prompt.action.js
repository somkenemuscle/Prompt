...
export async function createPrompt({ prompt, userId }) {
    try {
...
export async function getPromptById(promptId) {
    try {
        ...
        const prompt = await Prompt.findById(promptId).populate('author'); // Find the prompt by its ID
...
export async function updatePrompt({ prompt, userId }) {
    try {
        await connectToDatabase(); // Connect to MongoDB
...
export async function getAllPrompts() {
    try {
        ...
        const prompts = await Prompt.find().sort({ updatedAt: -1 }).populate('author') // Find all prompts, populate author field, and sort by updatedAt in descending order
...
export async function getUserPrompts(userId) {
    try {
        ...
        const prompts = await Prompt.find({ author: userId }).sort({ updatedAt: -1 }).populate('author'); // Find all prompts by the user and populate author field
...
export async function getUserPromptsByUsername(username) {
    try {
        ...
        const prompts = await Prompt.find({ author: user._id }).sort({ updatedAt: -1 }).populate('author');
